...
def convert_ldm_controlnet_checkpoint(checkpoint, config):
    """
    Takes a state dict and a config, and returns a converted checkpoint.
    """
    ...
    # Retrieves the keys for the output blocks only
    num_zero_conv_blocks = len(
        {
            ".".join(layer.split(".")[:2])
            for layer in unet_state_dict
            if "zero_convs" in layer
        }
    )
    for i in range(num_zero_conv_blocks):
        for content in ['weight', 'bias']:
            new_name = 'controlnet_down_blocks.{}.{}'.format(i, content)
            old_name = 'zero_convs.{}.0.{}'.format(i, content)
            new_checkpoint[new_name] = unet_state_dict[old_name]

    new_checkpoint['controlnet_mid_block.bias'] = unet_state_dict['middle_block_out.0.bias']
    new_checkpoint['controlnet_mid_block.weight'] = unet_state_dict['middle_block_out.0.weight']
    return new_checkpoint
